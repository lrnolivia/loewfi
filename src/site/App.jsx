...
import contactHeroUrl from '../../mockup/assets/img/contact-hero.jpg?url';
import {
...
const portfolioImg = (path, width = 1600) => `/media/${path.replace(/\.[^.]+$/, `-${width}.webp`)}`;
const portfolioImageProps = (path, sizes = '100vw') => ({
  src: portfolioImg(path),
  srcSet: `${portfolioImg(path, 720)} 720w, ${portfolioImg(path, 1600)} 1600w`,
  sizes,
});
...
function SiteLink({ href, onClick, ...props }) {
...
function ProjectGrid({ slugs }) {
  return <div className="work-grid">{slugs.map((slug) => {
    ...
    return <SiteLink href={routePath(slug)} className="teaser-card" key={slug}><div className="teaser-card__frame"><div className="teaser-card__frame-inner"><img {...portfolioImageProps(project.cover, '(max-width: 560px) 92vw, (max-width: 900px) 46vw, 30vw')} alt="" loading="lazy" decoding="async" /></div></div><div className="teaser-card__meta"><span className="teaser-card__track mono">{project.track}</span><h3 className="teaser-card__title">{project.title}</h3><p className="teaser-card__desc">{project.description}</p></div></SiteLink>;
  })}</div>;
}
...
function PhotographyGallery({ project }) {
  ...
  <div className={`carousel-thumbs${view === 'carousel' ? ' is-active' : ''}`}>{project.images.map((src,i) => <button type="button" key={src} className={i === current ? 'is-active' : ''} onClick={() => setCurrent(i)} aria-label={`Go to image ${i+1}`}><img src={portfolioImg(src, 720)} alt="" loading="lazy" decoding="async" /></button>)}</div></div>
...
function Plate({ src, portfolioSrc, caption, className='' }) { return <figure className={`plate ${className}`}><img {...(portfolioSrc ? portfolioImageProps(portfolioSrc) : { src: img(src) })} alt={caption} loading="lazy" decoding="async" /><figcaption>{caption}</figcaption></figure>; }
...
export default function ProductionSiteApp() {
  ...
  useEffect(() => {
    const syncRoute = () => setPage(pageKey());
